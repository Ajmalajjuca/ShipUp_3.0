...
export class SocketService {
  ...
  private async handleDeliveryPartnerConnection(socket: AuthenticatedSocket): Promise<void> {
    ...
    try {
      ...
      this.io.to('admins').emit('delivery_partner_online', {
        userId,
        timestamp: new Date(),
      });
  ...
  private async handleLocationUpdate(socket: AuthenticatedSocket, data: any): Promise<void> {
    ...
    try {
      ...
      this.io.to('admins').emit('delivery_partner_location', {
        userId,
  ...
  private async handleOnlineStatusUpdate(socket: AuthenticatedSocket, data: { isOnline: boolean }): Promise<void> {
    ...
    try {
      ...
      this.io.to('admins').emit('delivery_partner_status_change', {
        userId,
  ...
  private handleDisconnection(socket: AuthenticatedSocket): void {
    ...
    if (role === UserRole.DELIVERY_PARTNER) {
      ...
      // Notify admins
      this.io.to('admins').emit('delivery_partner_offline', {
        userId,
        timestamp: new Date(),
      });
    }
  ...
  public async broadcastToDeliveryPartners(event: string, data: any): Promise<void> {
    this.io.to('delivery_partners').emit(event, data);
  }
